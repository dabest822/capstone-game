...
# Only get children of PathSegments node
...
func _ready():
	# Make sure we only work with PathSegments children
	if path_segments.is_empty():
		push_warning("No path segments found under PathSegments node!")
		return
	...
	# Initialize shader parameters for segments only
	for segment in path_segments:
		if segment.material:
